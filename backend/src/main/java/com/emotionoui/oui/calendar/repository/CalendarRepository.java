...
public interface CalendarRepository extends JpaRepository<Emotion, Integer> {
    ...
    // 개인 일기 찾기
    @Query("SELECT e FROM Emotion e WHERE e.member.memberId = :memberId AND YEAR(e.date) = :year AND MONTH(e.date) =:month AND e.dailyDiary.diary.type =com.emotionoui.oui.diary.entity.DiaryType.개인 ORDER BY e.date")
    List<Emotion> findMyDiarybyDate(Integer memberId, Integer year, Integer month);


    // 공유 일기 찾기
    @Query("SELECT e FROM Emotion e WHERE e.member.memberId = :memberId AND YEAR(e.date) = :year AND MONTH(e.date) =:month AND e.dailyDiary.diary.id =:diaryId ORDER BY e.date")
