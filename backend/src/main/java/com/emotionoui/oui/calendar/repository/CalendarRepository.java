...
public interface CalendarRepository extends JpaRepository<Emotion, Integer> {
    ...
    // 공유 일기 찾기
    @Query("SELECT e FROM Emotion e WHERE e.member.memberId = :memberId AND YEAR(e.date) = :year AND MONTH(e.date) =:month AND e.dailyDiary.diary.type =com.emotionoui.oui.diary.entity.DiaryType.공유 AND e.dailyDiary.diary.id =:diaryId AND e.dailyDiary.diary.isDeleted = 0 ORDER BY e.date  ")
    ...
    // 공유 일정 찾기
    @Query("SELECT s FROM Schedule s WHERE s.member.memberId = :memberId AND YEAR(s.date) = :year AND MONTH(s.date) =:month AND s.diary.id =:diaryId AND s.isDeleted = 0 AND s.type =com.emotionoui.oui.schedule.entity.ScheduleType.공유 ORDER BY s.date")
    List<Schedule> findShareSchedulebyDate(Integer memberId, Integer year, Integer month, Integer diaryId);
